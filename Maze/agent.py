...
class GenericSearchAlgorithm(SearchAgent):
    ...
    def __init__(self):
        ...
        self.backpointers = {}
        self.reset()
        print "A GenericSearchAlgorithm agent is initialize!"
...
class IDAStarSearchAgent(SearchAgent):
    """
    Iterative Deepening Astar Search agent implementation
    """
    def __init__(self):
        """
        A new Agent
    ...
    def get_action(self, observations):
        r, c = observations[0], observations[1]
        ...
        for m, (dr,dc) in enumerate(MAZE_MOVES):
            ...
            if not observations[2 + m]: # can we go that way?
                if (r2,c2) not in self.visited:
                    self.back[(r2, c2)] = (r, c)
                    currentValue = self.heuristic(r2, c2) + self.costSoFar(r2, c2)
                    print("The current value for node (%d, %d) is %d, cutoff is %d" % (r2, c2, currentValue, self.cutoff))
                    self.miniValue.append(currentValue)
                    if currentValue <= self.cutoff:
                        self.backpointers[ (r2,c2) ] = (r,c)
                        self.enque((r2, c2))
        v = self.constraints.get_instance()
        if len(self.queue) == 0:
            self.cutoff = min(x for x in self.miniValue if x > self.cutoff)
            self.reset()
            v[0] = 4
            return v
        else:
            ...
            get_environment().mark_maze_blue(current[0], current[1])
        dr, dc = current[0] - r, current[1] - c
        action = get_action_index((dr, dc))
        if action is not None and observations[2 + action] == 0:
            print("action %d is taken by the agent from (%d %d)" % (action, r, c))
            v[0] = action
        else:
            get_environment().teleport(self, current[0], current[1])
            v[0] = 4
        return v
    ...
    def costSoFar(self, r, c):
        g = 0
        while (r, c) in self.back:
            (r, c) = self.back[(r, c)]
            """
            Assign the cost for each step taken as 2, otherwise it is not obvious to see
            the iterative deepeening process
            """
            g += 2
            if (r, c) == (0, 0):
                break
    ...
    def reset(self):
        get_environment().teleport(self, 0.0, 0.0)
        self.miniValue = []
        self.queue = []
        self.visited = set([])
        self.backpointers = {}
    ...
    def enque(self, cell):
        (r, c) = cell
        """
        print ("Heuristic is %d,  cost so far is %d" % (self.heuristic(r, c), self.costSoFar(r, c)))
        """
        heappush(self.queue, Cell(self.heuristic(r, c) + self.costSoFar(r, c), r, c))
